...
export function extractMarkerPrefixes(text) {
  const roadmap = [
    ...text.matchAll(/([A-Za-z0-9]+(?:-[A-Za-z0-9]+)*)-roadmap-id/g),
  ].map((match) => match[1]);
  const blockedBy = [
    ...text.matchAll(/([A-Za-z0-9]+(?:-[A-Za-z0-9]+)*)-blocked-by/g),
  ].map((match) => match[1]);
  return {
...
function checkMarkerPrefixes(root, report) {
  ...
  report.passes.push(
    `marker prefix is valid and consistent (${allPrefixes[0]})`,
  );
...
function checkProjectCommands(root, report) {
  ...
  try {
    ...
    report.errors.push(
      'cannot read .github/instructions/idd-overview.instructions.md',
    );
    return null;
  }
