...
2. If the target issue carries the configured roadmap label or an
   `dantalion-web-demo-roadmap-id` marker — the same test
   **A2**'s roadmap-node/execution-leaf classification rule uses (an
   ...
   - Apply **A3**'s dependency bullet to the target itself (both
     ...
     `dantalion-web-demo-blocked-by` markers) and its
...
NOT contain an `dantalion-web-demo-roadmap-id` marker (not itself
a roadmap) or an `dantalion-web-demo-blocked-by` marker, AND
...
`dantalion-web-demo-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`dantalion-web-demo-roadmap-id` marker. Retro-label a legacy
...
`<!-- dantalion-web-demo-roadmap-id: ... -->` marker are
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `dantalion-web-demo-blocked-by` markers on the explicit target.
...
- **A0-O only** (when `issue-scope` is `orphan-first`, or when
  ...
  `dantalion-web-demo-roadmap-id` or
  `dantalion-web-demo-blocked-by` markers.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `dantalion-web-demo-roadmap-id` marker when checking
  `dantalion-web-demo-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- dantalion-web-demo-blocked-by: {roadmap-id} -->` markers —
  find the issue whose body contains a matching
  `<!-- dantalion-web-demo-roadmap-id: {roadmap-id} -->`; treat as
...
`<!-- dantalion-web-demo-autopilot-suitability: N -->` footer, or the
...
Read the authored `<!-- dantalion-web-demo-effort: S|M|L -->` footer
...
- **Roadmap identity** (`dantalion-web-demo-roadmap-id`): in the
...
- **Sequential dependency** (`dantalion-web-demo-blocked-by`): in an
  issue body — this issue **cannot start until** the roadmap with the
  matching `roadmap-id` is closed.

**Do not use `dantalion-web-demo-blocked-by` to group sub-tasks under
