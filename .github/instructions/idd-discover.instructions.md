...
- Does NOT contain any
  `<!-- dantalion-web-demo-roadmap-id: … -->` marker (the issue
  is not itself a roadmap).
- Does NOT contain any
  `<!-- dantalion-web-demo-blocked-by: … -->` marker.
...
**A0-T** (the scoped `dantalion-web-demo-roadmap-id` lookup
...
for `dantalion-web-demo-roadmap-id` to resolve
`dantalion-web-demo-blocked-by` dependency markers; see A2 for
...
or containing an
`<!-- dantalion-web-demo-roadmap-id: ... -->` marker is a
...
- All dependency issues are closed or otherwise completed. Check both
  ...
  `<!-- dantalion-web-demo-roadmap-id: {roadmap-id} -->`. If that
...
4. **A3 filtered to zero** (A2 found execution candidates but all were
   ...
   **Diagnostic — all candidates blocked by an open roadmap**: if every
   ...
   `<!-- dantalion-web-demo-blocked-by: X -->` marker that points
...
- **Roadmap identity** (`dantalion-web-demo-roadmap-id`): placed
...
- **Sequential dependency** (`dantalion-web-demo-blocked-by`):
...
**Do not use `dantalion-web-demo-blocked-by` to group sub-tasks
